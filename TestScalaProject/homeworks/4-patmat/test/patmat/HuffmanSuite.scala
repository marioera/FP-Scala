...
class HuffmanSuite extends FunSuite {
  ...
  test("decode and encode a very short text should be identity") {
    new TestTrees {
      ...
      assert(decode(t1, encode(t1)("ababaaabbb".toList)) === "ababaaabbb".toList)
    }
  }

  test("decode and encode french tree") {
    new TestData {
      assert(encode(frenchCode)("huffmanestcool".toList) === secret)
      assert(decode(frenchCode, encode(frenchCode)("mariofunwithscala".toList)) === "mariofunwithscala".toList)
      assert(decode(frenchCode, encode(frenchCode)("huffmanestcool".toList)) === "huffmanestcool".toList)
    }
  }

  test("convert small trees") {
    new TestTrees {
  ...
  test("decode and quickEnconde french tree") {
    new TestData {
      assert(quickEncode(frenchCode)("huffmanestcool".toList) === secret)
      assert(decode(frenchCode, quickEncode(frenchCode)("mariofunwithscala".toList)) === "mariofunwithscala".toList)
