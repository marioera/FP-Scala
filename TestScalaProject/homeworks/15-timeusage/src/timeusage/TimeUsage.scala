...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def loop_classifiedColumns(columnNames: List[String], primaryCols: List[Column], workingCols: List[Column], otherCols: List[Column]): (List[Column], List[Column], List[Column]) = columnNames match {
      case Nil => (primaryCols, workingCols, otherCols)
      case h :: t =>
        if (h.startsWith("t01") || h.startsWith("t03") || h.startsWith("t11") || h.startsWith("t1801") || h.startsWith("t1803"))
          loop_classifiedColumns(t, col(h) :: primaryCols, workingCols, otherCols)
        else if (h.startsWith("t05") || h.startsWith("t1805"))
    ...
    loop_classifiedColumns(columnNames, List[Column](), List[Column](), List[Column]())
  }
  ...
  def timeUsageSummary(primaryNeedsColumns: List[Column], workColumns: List[Column], otherColumns: List[Column], df: DataFrame): DataFrame = {
    ...
    def loop_sum(columns: List[Column], sum: Column): Column = columns match {
      case Nil => sum
      case h :: t => loop_sum(t, sum + df(h.toString()))
    }

    val primaryNeedsProjection: Column = (loop_sum(primaryNeedsColumns, lit(0.0d)) / 60.0).as("primaryNeeds")
    val workProjection: Column = (loop_sum(workColumns, lit(0.0d)) / 60.0).as("work")
    val otherProjection: Column = (loop_sum(otherColumns, lit(0.0d)) / 60.0).as("other")
    df
