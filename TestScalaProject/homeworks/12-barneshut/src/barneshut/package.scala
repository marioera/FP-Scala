...
package object barneshut {
  ...
  case class Fork(nw: Quad, ne: Quad, sw: Quad, se: Quad) extends Quad {
    ...
    def insert(b: Body): Fork = {
      val relPos = (b.x < centerX, b.y < centerY)
      relPos match {
  ...
  //Fork(Leaf(0.5,0.5,1.0,List(barneshut.package$Body@3d51f06e)),Leaf(1.5,0.5,1.0,List(barneshut.package$Body@7ed7259e)),Empty(0.5,1.5,1.0),Empty(1.5,1.5,1.0))
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body])
    ...
    def insert(b: Body): Quad = {
      if (size <= minimumSize) Leaf(centerX, centerY, size, bodies :+ b)
        ...
        //(bodies :+ b).foreach(newFork.insert)
        (bodies :+ b).foldLeft(Fork(nw, ne, sw, se))((q, body) => q.insert(body))
      }
    }
  }
