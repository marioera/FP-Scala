...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    @tailrec
    def loopLos(idx: Int, maxAngle: Float): Unit = {
      if (idx < length) {
        val newMax = math.max(maxAngle, input(idx) / idx)
        output(idx) = newMax
        loopLos(idx + 1, newMax)
      }
    }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    @tailrec
    def loopLos(idx: Int, maxAngle: Float): Float = {
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    if (from < until) {
      val newMax = math.max(startingAngle, input(from) / from)
      output(from) = newMax
