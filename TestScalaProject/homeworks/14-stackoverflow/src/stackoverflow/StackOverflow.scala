...
class StackOverflow extends Serializable {
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val newClusters = vectors.map(vector => (findClosest(vector, means), vector))
      .groupByKey()
  ...
  def calcMedian(xs: List[Int]) = {
    ...
    val (lower, upper) = xs.sortWith(_ < _).splitAt(length / 2)
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langPercent: Double = vs.count(_._1 == langIdx) * 100d / clusterSize // percent of the questions in the most common language
      val medianScore: Int = calcMedian(vs.map(_._2).toList)
